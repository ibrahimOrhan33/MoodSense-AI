...
from openai import OpenAI # Google kütüphanesi yerine OpenAI eklendi
...
# 1. AYARLAR
# ==========================================
# BURAYA OPENAI API KEY'İNİ YAPIŞTIR (sk-... ile başlayan)
API_KEY = "This place for your API" 
...
AI_AKTIF = False
client = None # OpenAI İstemcisi

try:
    if "sk-" in API_KEY: # Basit bir kontrol
        client = OpenAI(api_key=API_KEY)
        AI_AKTIF = True
    else:
        print("UYARI: Geçerli bir OpenAI API Key girilmedi.")
...
# ==========================================
# 4. CHAT THREAD (DÜZENLENDİ: OpenAI)
# ==========================================
class ChatThread(QThread):
    ...
    def __init__(self, user_message):
        ...
        # Artık session taşımıyoruz, client globalden kullanılıyor
    ...
    def run(self):
        try:
            ...
            try:
                # Google Search çalışması için internet gerekir
            ...
            # OpenAI için Sistem ve Kullanıcı mesajlarını ayırıyoruz
            system_prompt = "Sen MoodSense asistanısın. Sana verilen finans ve web verilerini kullanarak veya genel kültürünle samimi, kısa ve öz cevap ver."
            ...
            self.response_signal.emit(cevap)
...
class MoodSenseWindow(QMainWindow):
    ...
    def giris_basarili(self, isim, duygu):
        ...
        if AI_AKTIF:
            ...
            self.chat_ekle("MoodSense", mesaj)
             ...
             self.chat_ekle("Sistem", f"Hoş geldin {isim}. AI anahtarı girilmediği için çevrimdışı moddasın.")
    ...
    def mesaj_gonder_baslat(self):
        ...
        if AI_AKTIF:
            self.chat_worker = ChatThread(text) # OpenAI session gerektirmez, sadece text
            self.chat_worker.response_signal.connect(self.ai_cevap_geldi)
            self.chat_worker.start()
        else:
            self.chat_ekle("Sistem", "AI Bağlantısı Yok (API Key eksik).")
            self.msg_input.setEnabled(True)
